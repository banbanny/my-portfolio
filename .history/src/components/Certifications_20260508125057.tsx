...
function CertFile({ cert, onSelect }: { cert: Cert; onSelect: () => void }) {
  ...
  return (
    ...
    >
      <div style={{ width:"100%", aspectRatio:"16/10", overflow:"hidden", background:"#f5f5f5", position:"relative" }}>
        <img
          src={cert.image}
          alt={cert.label}
          style={{ width:"100%", height:"100%", objectFit:"contain", display:"block", transform: hovered ? "scale(1.03)" : "scale(1)", transition:"transform 0.3s ease" }}
          onError={(e) => {
      ...
      <div style={{ padding:"10px 12px 13px" }}>
        <p style={{ fontSize:"10px", color:"rgba(255,255,255,0.85)", margin:"0 0 3px", fontWeight:700, letterSpacing:"0.04em", overflow:"hidden", textOverflow:"ellipsis", display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical", lineHeight:1.4 }}>
          {cert.label}
        </p>
...
function FolderModal({ onClose, onSelect }: { onClose: () => void; onSelect: (cert: Cert) => void }) {
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:100, background:"rgba(0,0,0,0.88)", display:"flex", alignItems:"center", justifyContent:"center", padding:"16px", backdropFilter:"blur(5px)" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background:"#1a1a1a", borderRadius:"14px", border:"1px solid rgba(255,255,255,0.08)", width:"100%", maxWidth:"860px", height:"90vh", display:"flex", flexDirection:"column", overflow:"hidden" }}>
  ...
  return (
    <>
      <style>{`
        ...
        .cert-title.in-view {
          animation: smoothPopUp 0.7s cubic-bezier(0.22, 1, 0.36, 1) 0.1s forwards;
        }
        ...
        .cert-line.in-view { animation: drawLine 0.5s ease 0.6s forwards; }
        ...
        <div className="relative max-w-6xl mx-auto px-5 sm:px-6">
          <div className="flex flex-col md:flex-row gap-10 md:gap-12 items-center">
            ...
            <div className="flex-1 text-center md:text-left">
              <h2
                className={`cert-title${inView ? " in-view" : ""} uppercase text-white mb-2`}
                style={{ fontWeight:900, fontSize:"clamp(2rem, 7vw, 5rem)", letterSpacing:"0.02em" }}
              >
                CERTIFICATIONS
              </h2>
              <div className={`cert-line${inView ? " in-view" : ""}`} />
              <p
                className={`cert-fadein${inView ? " in-view" : ""} mx-auto md:mx-0`}
                style={{ fontSize:"clamp(13px, 1.8vw, 16px)", fontWeight:500, color:"rgba(255,255,255,0.38)", lineHeight:1.8, marginBottom:"20px", maxWidth:"420px", animationDelay:"0.7s" }}
              >
                Here are the certificates I received and passed from online courses.
              </p>
              <div
                className={`cert-fadein${inView ? " in-view" : ""} flex items-center gap-2 justify-center md:justify-start`}
