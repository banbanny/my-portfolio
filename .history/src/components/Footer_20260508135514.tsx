    ...
    <>
      <footer ref={ref} id="contact" className="relative min-h-[85vh] flex flex-col overflow-hidden bg-[#1B1919]">
        <style>{`
          ...
          .icon-btn:hover {
            color: #fff;
            border-color: rgba(244,52,52,0.6);
            background: rgba(244,52,52,0.1);
        ...
        <div className="relative flex-1 flex items-center">
          <div className="max-w-6xl mx-auto px-5 sm:px-6 w-full">
            <div className="flex flex-col items-center text-center md:flex-row md:items-center md:justify-between md:text-left gap-8 md:gap-10">
              ...
              {/* CTA heading */}
              <h2 className={`uppercase leading-tight ${visible ? "footer-cta" : ""}`}
                style={{ fontSize:"clamp(2rem, 7vw, 4.5rem)", fontWeight:900, fontFamily:"'Roboto', sans-serif", opacity: visible ? undefined : 0 }}>
                <span style={{ color:"#F43434" }}>WANT TO</span><br />
                <span className="text-white">COLLABORATE?</span>
              </h2>

              {/* Contact buttons */}
              <div className={`flex flex-col items-center md:items-end gap-4 ${visible ? "footer-links" : ""}`}
                ...
                {/* Row 3 — GitHub icon + LinkedIn icon + CV button */}
                <div className="flex items-center gap-3">

                  {/* GitHub icon only */}
                  <a href="https://github.com/banbanny" target="_blank" rel="noopener noreferrer" className="icon-btn" title="GitHub">
                  ...
                  {/* LinkedIn icon only */}
                  <a href="https://www.linkedin.com/in/YOUR-LINKEDIN-USERNAME" target="_blank" rel="noopener noreferrer" className="icon-btn" title="LinkedIn">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                  ...
                  {/* CV button — text */}
                  <button onClick={() => setShowCV(true)} className="contact-btn" style={{ gap:"6px" }}>
        ...
        <div className={`relative w-full pb-6 px-5 sm:px-6 ${visible ? "footer-copyright" : ""}`} style={{ opacity: visible ? undefined : 0 }}>
          <div className="max-w-6xl mx-auto">
            ...
            <p className="text-[10px] text-gray-500 tracking-[0.3em] uppercase text-center" style={{ fontFamily:"'Roboto', sans-serif" }}>
      ...
      {showCV && (
        <div
          ...
          style={{ position:"fixed", inset:0, zIndex:9999, background:"rgba(10,8,8,0.92)", backdropFilter:"blur(14px)", display:"flex", alignItems:"center", justifyContent:"center", padding:"16px" }}
        >
          <div
            className="cv-modal-inner"
            onClick={e => e.stopPropagation()}
            style={{ width:"100%", maxWidth:"780px", height:"90vh", background:"#111", border:"1px solid rgba(244,52,52,0.25)", borderRadius:"10px", overflow:"hidden", display:"flex", flexDirection:"column", boxShadow:"0 0 60px rgba(244,52,52,0.12)" }}
          >
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 18px", borderBottom:"1px solid rgba(244,52,52,0.15)", flexShrink:0 }}>
              <span style={{ fontFamily:"'Roboto', sans-serif", fontSize:"14px", fontWeight:"bold", letterSpacing:"0.2em", color:"#F43434", textTransform:"uppercase" }}>RESUME</span>
              <button onClick={() => setShowCV(false)} style={{ background:"none", border:"1px solid rgba(255,255,255,0.15)", borderRadius:"4px", padding:"4px 12px", color:"#9ca3af", fontFamily:"'Roboto', sans-serif", fontSize:"11px", cursor:"pointer", letterSpacing:"0.1em" }}>✕ CLOSE</button>
            </div>
            <div style={{ flex:1, overflowY:"auto", background:"#fff" }}>
              <img src="/cv-page1.png" alt="CV Page 1" style={{ width:"100%", display:"block" }} />
              <img src="/cv-page2.png" alt="CV Page 2" style={{ width:"100%", display:"block" }} />
            </div>
          </div>
        </div>
      )}
