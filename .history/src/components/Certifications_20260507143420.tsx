...
const certifications = [
  { id: 1, label: "Build Complete CMS Blog in PHP MySQLL Bootstrap & PDO", issuer: "Issuer Name", year: "2024", image: "/cert1.jpg" },
  ...
  return (
    <>
      <style>{`
        ...
        .cert-letter.in-view {
          animation: certLetterDrop 0.5s cubic-bezier(0.22,1,0.36,1) forwards;
        }
        ...
        .folder-shake {
          animation: folderShake 0.6s cubic-bezier(0.36,0.07,0.19,0.97) both;
        }

        /* Fade in up for rest of content */
        @keyframes fadeInUp {
        ...
        <div className="relative max-w-6xl mx-auto px-5 sm:px-6">
          <div className="flex flex-col md:flex-row gap-10 md:gap-12 items-center">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              {/* Animated title — each letter drops in */}
              <h2 className="uppercase text-white mb-2" style={{ fontWeight:900, fontSize:"clamp(2rem, 7vw, 5rem)", letterSpacing:"0.02em" }}>
                {"CERTIFICATIONS".split("").map((char, i) => (
                  <span
                    key={i}
                    className={`cert-letter${inView ? " in-view" : ""}`}
              ...
              {/* Underline draws in after letters */}
              ...
              <div className={`cert-fadein${inView ? " in-view" : ""} flex items-center gap-2 justify-center md:justify-start`}
            ...
            {/* RIGHT — Folder with tumble shake on hover */}
            <div className="shrink-0 flex items-center justify-center">
              ...
              >
                <span
                  className={isShaking ? "folder-shake" : ""}
                  style={{ fontSize:"clamp(80px, 15vw, 130px)", lineHeight:1, display:"block" }}
                >
                  📁
                </span>
      ...
      {folderOpen && (
        <FolderModal
          ...
          onSelect={(cert) => { const idx = certifications.findIndex((c) => c.id === cert.id); setSelectedIndex(idx); }}
        />
      )}
      {selectedIndex !== null && (
        <Lightbox
