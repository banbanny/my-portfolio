...
const projects: Project[] = [
  ...
  },
  {
    ...
    title: "Flash Review",
    type: "app",
     description: "Flash Review is a study companion designed to help students learn more effectively through interactive flashcards. It encourages active recall and repetition, making it easier to remember lessons, terms, and important concepts. With a simple and focused interface, the app motivates students to build consistent study habits, review at their own pace, and turn study sessions into a more engaging and productive experience.",
    thumbnail: "/flashreview.png",
    // ← Add your app screen images here
    screenshots: ["/Home.png", "/Create.png", "/ReviewerSet.png", "/Review.png"],
  },
  {
    id: 7,
    title: "Poster Design",
...
function AppModal({ project, onClose }: { project: Project; onClose: () => void }) {
  ...
  return (
    <div
      onClick={onClose}
      style={{ position:"fixed", inset:0, zIndex:200, background:"rgba(0,0,0,0.92)", display:"flex", alignItems:"center", justifyContent:"center", padding:"16px", backdropFilter:"blur(6px)" }}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        style={{ maxWidth:"900px", width:"100%", borderRadius:"14px", overflow:"hidden", border:"1px solid rgba(26,115,232,0.35)" }}
      >
        {/* Header */}
...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <>
      <div
        ref={ref}
        style={{ opacity: inView ? 1 : 0, transform: inView ? "translateY(0)" : "translateY(32px)", transition: `opacity 0.55s ease ${index * 0.08}s, transform 0.55s ease ${index * 0.08}s` }}
      >
        <div
        ...
        >
          {/* Thumbnail — same for all types */}
          <div style={{ position:"relative", width:"100%", aspectRatio:"16/9", overflow:"hidden" }}>
            <img
              src={project.thumbnail} alt="" aria-hidden="true"
              style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"cover", filter:"blur(18px) brightness(0.4) saturate(1.4)", transform:"scale(1.15)", pointerEvents:"none" }}
            />
            <img
              src={project.thumbnail} alt={project.title}
              style={{ position:"relative", zIndex:1, width:"100%", height:"100%", objectFit:"contain", transform: hovered ? "scale(1.04)" : "scale(1)", transition:"transform 0.5s ease", display:"block" }}
            ...
            {/* Type badge */}
            <span style={{ position:"absolute", top:"10px", left:"10px", zIndex:3, fontSize:"10px", fontFamily:"monospace", letterSpacing:"0.08em", textTransform:"uppercase", color:"#fff", background:cfg.badge, padding:"3px 10px", borderRadius:"4px" }}>
              {cfg.label}
            </span>
          </div>
